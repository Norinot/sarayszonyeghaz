...
export class UploadProductComponent implements OnInit {
          ...
          this.productForm
            ...
            ?.setValue(
              this.categories.find(
                (category) =>
                  category.code === response.category
              )
            )
  ...
  onSubmit(): void {
    ...
    const fields = [
      { key: 'category', value: this.getControl('category').value?.code },
      { key: 'name', value: this.getControl('name').value },
      { key: 'size', value: JSON.stringify(this.getControl('size').value), skipStringify: true },
      { key: 'material', value: this.getControl('material').value },
      { key: 'color', value: this.getControl('color').value },
      { key: 'design', value: this.getControl('design').value },
      { key: 'origin', value: this.getControl('origin').value },
      { key: 'cleaning', value: this.getControl('cleaning').value },
      { key: 'price', value: this.getControl('price').value }
    ];
    const product = new FormData()

    fields.forEach(({ key, value, skipStringify }) => {
      if (value) {
        product.append(key, skipStringify ? value : JSON.stringify(value));
      }
    });
    ...
    if (this.id != null) {
      this.productService
        ...
        .subscribe({
          ...
          error: (error) => {
            this.toastr.error(
              `Probléma történt a termék módosítása közben, ${error}`
            )
          },
        })
      ...
      this.productService.createNewProduct(product).subscribe({
        ...
        error: (error) => {
          this.toastr.error(
            `Probléma történt a termék hozzáadása közben, ${error}`
          )
        },
      })
    } else {
